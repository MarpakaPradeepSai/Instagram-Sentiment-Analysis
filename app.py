...
# Download each file
for file in files:
    download_file_from_github(f"{repo_url}/{file}", f"./distillbert_model/{file}")

# Load tokenizer and model
tokenizer = BertTokenizer.from_pretrained('./distillbert_model')
model = DistilBertForSequenceClassification.from_pretrained('./distillbert_model')
...
st.markdown(
    """
    <h1 style="font-size: 41px; text-align: center;">Instagram Sentiment Analysis with DistilBERT</h1>
    """,
    unsafe_allow_html=True
)
